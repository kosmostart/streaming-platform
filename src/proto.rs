...
impl<T, R> MagicBall<T, R> where T: Debug, T: serde::Serialize, for<'de> T: serde::Deserialize<'de>, R: Debug, R: serde::Serialize, for<'de> R: serde::Deserialize<'de> {
    pub fn new(addr: String, sender: Sender, rx: crossbeam::channel::Receiver<Vec<u8>>) -> MagicBall<T, R> {
        MagicBall {
...
impl MagicBall2 {
    pub fn new(addr: String, sender: Sender, rx: crossbeam::channel::Receiver<Vec<u8>>) -> MagicBall2 {
        MagicBall2 {
            addr,
            sender,
            rx
